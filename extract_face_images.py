"""
python extract_face.py --input_folder "/shika_data4/shivam/keyframe_split_remaining/" --output_folder "/shika_data4/shivam/keyframe_face_remaining"
...
def enhance_face_with_realesrgan(face, model_name='RealESRGAN_x4plus', outscale=4, denoise_strength=0.5, tile=0, tile_pad=10, pre_pad=0):
    """
...
def detect_and_save_faces(input_folder, output_folder, keywords, device, padding_ratio=0.20,
    ...
    """
    Detects faces in images, enhances the faces using Real-ESRGAN, and saves the enhanced faces as images.

    Parameters:
    input_folder (str): Directory containing input images.
    output_folder (str): Directory to save enhanced face images.
    keywords (list): List of keywords to filter the images.
            ...
            if boxes is not None:
                for i, (box, prob, landmark) in enumerate(zip(boxes, probs, landmarks)):
                    ...
                    if not is_blurry(face_with_surrounding, blur_threshold):
                        ...
                        enhanced_face = enhance_face_with_realesrgan(face_with_surrounding, model_name, outscale, denoise_strength, tile, tile_pad, pre_pad)
...
def main():
    parser = argparse.ArgumentParser(description="Detect and save faces from images with enhancement.")
    parser.add_argument('--input_folder', type=str, default='/shika_data4/shivam/keyframe_split_remaining', help='Path to the input folder containing images.')
    ...
    parser.add_argument('--padding_ratio', type=float, default=0.20, help='Padding ratio around the detected face.')
    ...
    parser.add_argument('--min_prob', type=float, default=0.90, help='Minimum probability threshold for face detection.')
