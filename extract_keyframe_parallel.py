...
def process_and_save(video_file, video_folder, save_at, device, model, soundnet):
    try:
        key_frame, audio = process_video(os.path.join(video_folder, video_file), device, model, soundnet)
...
if __name__ == "__main__":
    ...
    save_at = '/shika_data4/shivam/audioset_1/'

    # shika
    # video_folder = '/backup/data3/shivam/audio-visual-dataset/unbalanced_train_1'
    # save_at = '/data4/shivam/audioset/'
    ...
    error_count = 0
    max_workers = 16 # Set the number of concurrent threads

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_and_save, video_file, video_folder, save_at, device, model, soundnet): video_file for video_file in video_files}

        for future in tqdm(concurrent.futures.as_completed(futures), total=len(video_files), desc="Processing"):
            video_file = futures[future]
            try:
                video_file, error = future.result()
                if error:
                    print(f"Error in processing video {video_file}: {error}")
                    error_count += 1
            except Exception as e:
                print(f"Unhandled error in processing video {video_file}: {e}")
                error_count += 1
