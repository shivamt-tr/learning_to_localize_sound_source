#!/bin/bash
# download AVSpeech dataset using yt-dlp (run this on other servers)
#
...
# example run on servers:
# njobs=100 bash download_updated.sh unbalanced_train_split_7.csv unbalanced_train_7
...
# command definitions
youtubedl="yt-dlp --quiet --no-warnings"
...
download_video() {
	# parse csv
	IFS=',' read -r ytid start end label <<< "$1"
	# IFS=',' read -r ytid start <<< "$1"
	# IFS=',' read -r ytid start end <<< "$1"

	# id=${ytid}_${start}
	id=${ytid}_${label}
	filename="$id.mp4"

	# outdir=$outdir/$mode/$label  # save train and test in separate folders
	...
	duration="$(bc <<< "10")"  # duration is 10 seconds in vggsound
...
cat "$csvfile" | $parallel -d "\r\n" -j "$njobs" --timeout 600 download_video
