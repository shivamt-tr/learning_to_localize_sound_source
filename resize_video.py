...
def process_video(video_file, args):
    ...
    try:
        ...
        writer = imageio.get_writer(os.path.join(args.save_dir, "resized_video", f"{video_name}.mp4"), mode='I', fps=fps)
...
def main(args):
    ...
    with Pool(processes=args.num_workers) as pool:
        results = list(tqdm(pool.imap(lambda video_file: process_video(video_file, args), video_files), total=len(video_files), desc="Processing"))
